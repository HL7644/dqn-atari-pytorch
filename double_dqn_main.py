...
class Double_DQN():
  def __init__(self, agent):
    self.agent=agent #use same agent as original DQN
  ...
  def get_value_loss(self, batch_data):
    ...
    for ep_step in batch_data:
      ...
      #choose argmax w/ current qm
      for a_idx in self.agent.a_idx_list:
        Q_values[a_idx]=self.agent.qm(pre_obs_f, a_idx)
      argmax_idx=torch.argmax(Q_values, dim=0)
      #compute value w/ target qm
      Q_max=self.agent.target_qm(pre_obs_f, argmax_idx)
  ...
  def test_performance(self, back_track): #follow qm while testing
    acc_rewards=[]
    a_mode='policy'
    ...
    for epoch in range(1, n_epochs+1): #perform updates on every step (sampling and updating)
      while True:
        if step%steps_per_epoch==0:
          break
        if step>start_after: #implement policy after some steps
          a_mode='policy'
        if step>update_after: #update after some data has been collected
          update=True
...
double_dqn=Double_DQN(agent)
double_dqn.train(64, 100, 4000, 10000, 1000, 50, 4, 4, 0.1, 1e-4)